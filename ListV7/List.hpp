...
public:
     ...
     */
    Node( B x, List<B> xs = EMPTY<B> ) : _datum( x ), _tail( xs ) {}
...
private:
    ...
    friend std::shared_ptr<Node>& std::shared_ptr<Node>::operator= (std::shared_ptr<Node> const&) noexcept;
    ...
    template <typename A> friend List<A> operator| ( A, List<A> );
