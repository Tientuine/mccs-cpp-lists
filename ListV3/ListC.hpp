...
class List
{
  ...
  struct Node
  {
     ...
     */
    Node(T x, Node* xs = nullptr) : _datum(x), _tail(xs) {}
    /**
...
public:
   ...
   *
   */
  List(List const& xs) : List(xs._head) {}

  /**
   *
   */
  List(List&& xs) noexcept
   : _head(xs._head) { xs._head = nullptr; }
   ...
   : List(xs._head)
  {
    ...
    this->_head = reinterpret_cast<Node*>(malloc(sizeof(Node)));
    this->_head->_datum = x;
    this->_head->_tail = oldhead;
  }
  ...
  List& operator= ( List const& xs )
  {
    ...
    Node* n = xs._head;
    if (n) {
      this->_head = reinterpret_cast<Node*>(calloc(1,sizeof(Node)));
      this->_head->_datum = n->_datum;
      Node* copy = this->_head;
      while ((n = n->_tail)) {
        copy->_tail = reinterpret_cast<Node*>(calloc(1,sizeof(Node)));
        copy->_tail->_datum = n->_datum;
        copy = copy->_tail;
      }
    }
  }
...
auto map( F f, List<U> const& xs ) -> List<decltype(f(head(xs)))>
{
  ...
  while ( (from = from->_tail) ) {
    to->_tail = reinterpret_cast<Node*>(calloc(1,sizeof(Node)));
    to->_tail->_datum = f( from->_datum );
    to = to->_tail;
  }
  return ys;
}
