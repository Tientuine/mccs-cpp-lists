...
class List
{
  ...
  struct Node
  {
     ...
     */
    Node( T x, Ptr const& xs ) : _datum( x ), _tail( xs ) {}

    /**
     * Make an internal list node from an element and a pointer to a node.
     * @note This ctor overload implements move semantics for r-value pointers
     * @param x  an element
     * @param xs temporary pointer to an existing node (or nullptr)
     */
    Node( T x, Ptr && xs ) : _datum( x ), _tail( std::forward<Ptr>( xs ) ) {}
...
public:
   ...
   * @note this constructor will never be called implicitly
   */
  explicit List( T const& x, List && xs )
    : _head( std::make_shared<Node>( x, std::move( xs._head ) ) ) {}
   ...
   * @note The new and old lists will share the same tail.
   * @param xs an existing list
   */
  List( List const& xs ) : _head( xs._head ) {}

  /**
   * Move a list to a new instance.
   * @param xs a temporary list
   */
  List( List && xs ) : _head( std::move( xs._head ) ) {}

  /**
   * Copy the list on the right-hand side.
   * @note Afterward the two lists will have the same tails.
   * @param xs an existing list
   */
  List& operator= ( List const& xs ) { this->_head = xs._head; return *this; }

  /**
   * Move a list to overwrite another existing list.
   * @param xs a temporary list
   */
  List& operator= ( List && xs ) {
    this->_head = std::move( xs._head ); return *this;
  }
...
private:
    ...
    template <typename U> friend std::ostream& operator<< ( std::ostream& os, List<U> const& xs );
};
 ...
 */
template <typename U>
List<U> operator| ( U x, List<U> && xs ) {
  return List<U>( x, std::forward<List<U>>( xs ) );
}
...
auto map( F f, List<U> const& xs ) -> List<decltype(f(head(xs)))>
{
  ...
  return f( head( xs ) ) | map<F,U>( f, tail( xs ) );
}
...
{
  if ( null( xs ) ) { return List<U>::EMPTY; }
...
{
  if ( k == 0 || null( xs ) ) { return List<U>::EMPTY; }
...
{
  std::function<List<U>(List<U> const&, List<U>)> rev;
  rev = [&rev](List<U> const& xs, List<U> ys)->List<U>{
    if ( null( xs ) ) { return ys; }
    return rev( tail( xs ), head( xs ) | ys );
  };
  return rev( xs, List<U>::EMPTY );
}
